...
if [ ! -s /dashboard/damon.conf ]; then
  ...
  [[ -z "$GH_USER" || -z "$GH_CLIENTID" || -z "$GH_CLIENTSECRET" || -z "$ARGO_AUTH" || -z "$AG_DOMAIN" ]] && error " There are variables that are not set. "
  ...
  [ -n "$GH_PROXY" ] && wget --server-response --quiet --output-document=/dev/null --no-check-certificate --tries=2 --timeout=3 https://raw.githubusercontent.com/vxzyfx/nezha-service/main/README.md >/dev/null 2>&1 && unset GH_PROXY
  ...
  if [ "$REVERSE_PROXY_MODE" = 'grpcwebproxy' ]; then
    wget -c ${GH_PROXY}https://github.com/vxzyfx/nezha-service/releases/download/grpcwebproxy/grpcwebproxy-linux-$ARCH.tar.gz -qO- | tar xz -C $WORK_DIR
...
EOF
  ...
  wget -P ${WORK_DIR}/data/ ${GH_PROXY}https://github.com/vxzyfx/nezha-service/raw/main/sqlite.db
...
EOF
  ...
  # 生成SSL证书
  echo -e $NEZHA_CERT_PEM > $WORK_DIR/nezha-dashboard.pem
  echo -e $NEZHA_KEY_PEM > $WORK_DIR/nezha-dashboard.key
  echo -e $NEZHA_AG_CERT_PEM > $WORK_DIR/nezha-ag.pem
  echo -e $NEZHA_AG_KEY_PEM > $WORK_DIR/nezha-ag.key
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/vxzyfx/nezha-service/main/template/backup.sh | sed '1,/^########/d' >> $WORK_DIR/backup.sh
...
EOF
    ...
    wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/vxzyfx/nezha-service/main/template/restore.sh | sed '1,/^########/d' >> $WORK_DIR/restore.sh
  fi
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/vxzyfx/nezha-service/main/template/renew.sh | sed '1,/^########/d' >> $WORK_DIR/renew.sh

  # 生成定时任务: 1.每天北京时间 3:30:00 更新备份和还原文件，2.每天北京时间 4:00:00 备份一次，并重启 cron 服务； 3.每分钟自动检测在线备份文件里的内容
  [ -z "$NO_AUTO_RENEW" ] && [ -s $WORK_DIR/renew.sh ] && ! grep -q "$WORK_DIR/renew.sh" /etc/crontab && echo "30 3 * * * bash $WORK_DIR/renew.sh" >> /etc/crontab 
  [ -s $WORK_DIR/backup.sh ] && ! grep -q "$WORK_DIR/backup.sh" /etc/crontab  && echo "0 4 * * * bash $WORK_DIR/backup.sh a" >> /etc/crontab 
  [ -s $WORK_DIR/restore.sh ] && ! grep -q "$WORK_DIR/restore.sh" /etc/crontab  && echo "* * * * * bash $WORK_DIR/restore.sh a" >> /etc/crontab 
